...
moments <- function(type, parms) {
  ...
  if (type == "probitnorm") {
    ...
    ex2 <- integrate(
      function(x) {
        x^2 * mcmapper::dprobitnorm(x, mu, sigma)
      },
      0,
      1
    )$value
...
inv_mean_quantile <- function(type, m, q, p) {
  if (type == "logitnorm") {
    res <- tryCatch(
      ...
      error = (function(cond) {
        logitnorm::twCoefLogitnormE(
          mean = m,
          quant = q,
          perc = p,
          theta0 = logitnorm::twCoefLogitnorm(m, q, p)
        )
      })
    )
...
calc_cstat <- function(type, parms, m = NULL) {
  ...
  if (type == "logitnorm") {
    ...
    C <- (1 -
      integrate(
        f = function(x) {
          mcmapper::plogitnorm(x, parms[1], parms[2])^2
        },
        lower = 0,
        upper = 1
      )$value -
  ...
  if (type == "beta") {
    if (is.null(m)) {
      m <- parms[1] / (parms[1] + parms[2])
    }
    C <- (1 -
      integrate(
        f = function(x) {
          pbeta(x, parms[1], parms[2])^2
        },
        lower = 0,
        upper = 1
      )$value -
  ...
  if (type == "probitnorm") {
    if (is.null(m)) {
      m <- pnorm(parms[1] / sqrt(1 + parms[2]^2))
    } #TODO: check
    C <- (1 -
      integrate(
        f = function(x) {
          mcmapper::pprobitnorm(x, parms[1], parms[2])^2
        },
        lower = 0,
        upper = 1
      )$value -
...
infer_cal_int_from_mean <- function(
  dist_type,
  dist_parms,
  cal_mean,
  cal_slp,
  ...
  if (dist_type == "logitnorm") {
    f <- function(intercept) {
      integrate(
        function(x, intercept, slope) {
          expit((logit(x) - intercept) / cal_slp) *
            mcmapper::dlogitnorm(x, dist_parms[1], dist_parms[2])
        },
        0,
        1,
        intercept = intercept,
  ...
  if (dist_type == "beta") {
    f <- function(intercept) {
      integrate(
        function(x, intercept, slope) {
          expit((logit(x) - intercept) / cal_slp) *
            dbeta(x, dist_parms[1], dist_parms[2])
        },
        0,
        1,
        intercept = intercept,
  ...
  if (dist_type == "probitnorm") {
    f <- function(intercept) {
      integrate(
        function(x, intercept, slope) {
          expit((logit(x) - intercept) / cal_slp) *
            mcmapper::dprobitnorm(x, dist_parms[1], dist_parms[2])
        },
        0,
        1,
        intercept = intercept,
...
infer_cal_int_from_oe <- function(
  dist_type,
  dist_parms,
  cal_oe,
  cal_slp,
...
calc_se_sp <- function(
  ...
  #TODO: prev should be optional
  hz <- expit(logit(threshold) * cal_slp + cal_int)

  if (dist_type == "logitnorm") {
    tp <- integrate(
      f = function(x) {
        x * mcmapper::dlogitnorm(x, dist_parms[1], dist_parms[2])
      },
      hz,
      1
    )$value
    se <- unname(tp / prev)
    sp <- unname(
      (mcmapper::plogitnorm(hz, dist_parms[1], dist_parms[2]) - (prev - tp)) /
        (1 - prev)
    )
  }
  ...
  if (dist_type == "beta") {
    tp <- integrate(
      f = function(x) {
        x * dbeta(x, dist_parms[1], dist_parms[2])
      },
      hz,
      1
    )$value
    se <- unname(tp / prev)
    sp <- unname(
      (pbeta(hz, dist_parms[1], dist_parms[2]) - (prev - tp)) / (1 - prev)
    )
  }
  ...
  if (dist_type == "probitnorm") {
    tp <- integrate(
      f = function(x) {
        x * mcmapper::dprobitnorm(x, dist_parms[1], dist_parms[2])
      },
      hz,
      1
    )$value
    se <- unname(tp / prev)
    sp <- unname(
      (mcmapper::pprobitnorm(hz, dist_parms[1], dist_parms[2]) - (prev - tp)) /
        (1 - prev)
    )
  }
...
infer_correlation <- function(
  ...
  for (i in 1:n_sim) {
    p <- do.call(
      paste0("r", dist_type),
      args = as.list(c(n, unname(dist_parms)))
    )
    Y <- rbinom(n, 1, p)
...
calc_riley_vars <- function(N, parms) {
  ...
  if (dist_type == "logitnorm") {
    ...
    I_a <- integrate(
      function(x) {
        mcmapper::dlogitnorm(x, dist_parms[1], dist_parms[2]) * (x * (1 - x))
      },
      0,
      1
    )$value
    I_b <- integrate(
      function(x) {
        mcmapper::dlogitnorm(x, dist_parms[1], dist_parms[2]) *
          (((logit(x) - cal_int) / cal_slp)^2 * (x * (1 - x)))
      },
      0,
      1
    )$value
    ...
    tryCatch(
      {
        ...
        I_a <- integrate(
          function(x) {
            dbeta(x, dist_parms[1], dist_parms[2]) * (x * (1 - x))
          },
          0,
          1
        )$value
  ...
  if (dist_type == "probitnorm") {
    ...
    I_a <- integrate(
      function(x) {
        mcmapper::dprobitnorm(x, dist_parms[1], dist_parms[2]) * (x * (1 - x))
      },
      0,
      1
    )$value
...
riley_samp <- function(target_ciws, parms) {
  ...
  if (!is.null(target_ciws[['cal_int']]) | !is.null(target_ciws[['cal_slp']])) {
    ...
    if (dist_type == "beta") {
      ...
      I_a <- integrate(
        function(x) {
          dbeta(expit(logit(x)), dist_parms[1], dist_parms[2]) *
            (exp(cal_int + x * cal_slp) / (1 + exp(cal_int + x * cal_slp))^2)
        },
        0,
        1
      )$value
      I_b <- integrate(
        function(x) {
          dbeta(expit(logit(x)), dist_parms[1], dist_parms[2]) *
            ((x^2 * exp(cal_int + x * cal_slp)) /
              (1 + exp(cal_int + x * cal_slp))^2)
        },
        0,
        1
      )$value
    ...
    if (dist_type == "probitnorm") {
      ...
      I_a <- integrate(
        function(x) {
          mcmapper::dprobitnorm(expit(logit(x)), dist_parms[1], dist_parms[2]) *
            (exp(cal_int + x * cal_slp) / (1 + exp(cal_int + x * cal_slp))^2)
        },
        0,
        1
    ...
    if (length(v1) > 0) {
      out$fciw.cal_int <- round(I_b / (v1 * (I_a * I_b - I_ab^2)))
    }
    if (length(v2) > 0) {
      out$fciw.cal_slp <- round(I_a / (v2 * (I_a * I_b - I_ab^2)))
    }
  }
  out
}
...
calc_ciw_2s <- function(N, parms) {
  ciw_cstat <- ciw_cal_oe <- ciw_cal_mean <- ciw_cal_int <- ciw_cal_slp <- rep(
    NA,
    length(N)
  )
  ...
  for (i in 1:l) {
    ...
    new_parms <- list(
      prev = rbeta(1, hat_prev[1], hat_prev[2]),
      cstat = f(),
...
calc_ciw_sample <- function(N, parms) {
  ...
  se_cstat <- se_cal_oe <- se_cal_mean <- se_cal_int <- se_cal_slp <- rep(
    NA,
    length(N)
  )
  ...
  for (i in 1:length(N)) {
    ...
    reg <- fastLogisticRegressionWrap::fast_logistic_regression(
      logit_pi[1:n, ],
      Y[1:n],
  ...
  list(
    cstat = k * se_cstat,
    cal_oe = k * se_cal_oe,
    cal_mean = k * se_cal_mean,
    cal_int = k * se_cal_int,
    cal_slp = k * se_cal_slp
  )
}
...
calc_ciw_mc <- function(N, parms_sample, method) {
  ...
  ciw_cstat <- ciw_cal_oe <- ciw_cal_mean <- ciw_cal_int <- ciw_cal_slp <- matrix(
    NA,
    nrow = n,
    ncol = length(N)
  )
...
find_n_mean <- function(target, N, ciws, decreasing = TRUE, convex = TRUE) {
  # require(cobs)
  ...
  constraint <- c(
    ifelse(decreasing, "decreasing", ""),
    ifelse(convex, "convex", "")
  )
...
find_n_quantile <- function(target, N, q, ciws) {
  ...
  f <- function(x) {
    predict(fit, newdata = data.frame(X = x)) - target
  }
  round(uniroot(f, c(min(N), max(N)))$root, 0)
}
...
plot_cal_instability <- function(
  ...
  for (i in 1:nrow(sample)) {
    p <- do.call(
      paste0("r", sample$dist_type[i]),
      list(N, sample$dist_parm1[i], sample$dist_parm2[i])
    )
    Y <- rbinom(N, 1, p)
...
plot_cal_distance <- function(N, sample, method = "loess", X = (1:99) / 100) {
  ...
  for (i in 1:nrow(sample)) {
    ...
    if (method == "loess") {
      fcl <- loess(Y ~ pi)
      out[i, ] <- predict(fcl, X) -
        expit(logit(X) * sample$cal_slp[i] + sample$cal_int[i])
    }
    if (method == "line") {
      reg <- glm(Y ~ logit(pi), family = binomial())
      out[i, ] <- predict(
        reg,
        newdata = data.frame(pi = X),
        type = "response"
      ) -
        expit(logit(X) * sample$cal_slp[i] + sample$cal_int[i])
    }
  ...
  for (i in 1:nrow(out)) {
    if (i == 1) {
      plot(
        ...
        ylim = ylim,
        xlab = "Simulated predicted risks",
        ylab = "Simulated calibration error",
        type = 'l',
        col = 'grey'
      )
    } else {
