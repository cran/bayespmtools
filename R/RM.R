...
find_n_RM <- function(
  ...
  if (is.null(base_parms)) {
    ...
    tmp <- mcmapper::mcmap(
      c(base_parms$prev, base_parms$cstat),
      type = base_parms$dist_type
    )$valu
    base_parms$dist_parm1 <- tmp[1]
    base_parms$dist_parm2 <- tmp[2]
  ...
  for (i in 1:n) {
    K <- base_ciws[[target_metrics[i]]] /
      target_values[[i]][length(target_values[[i]])]
  ...
  for (i in 1:n_sim) {
    ...
    for (j in 1:n) {
      if (target_rules[j] == "eciw") {
        working_Ns[j] <- working_Ns[j] +
          steps[j] / i * (y[[target_metrics[j]]][j] - target_values[[j]])
      }
      if (target_rules[j] == "qciw") {
        working_Ns[j] <- working_Ns[j] +
          steps[j] /
            i *
            ifelse(
              y[[target_metrics[j]]][j] > target_values[[j]][2],
              target_values[[j]][1],
              -(1 - target_values[[j]][1])
            )
      }
